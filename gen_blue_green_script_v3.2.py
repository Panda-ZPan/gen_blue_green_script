...
class BlueGreenGenerator:
    ...
    def generate_ingress(self, base_ingress: Dict, ingress_type: str, env: str) -> Dict:
        ...
        reenv = self.green_name if env == self.blue_name else self.blue_name
    ...
    def generate_all(self, output_root: str):
        ...
        green_recycle = self.generate_blue_green_deployment(self.green_name)
        green_recycle['spec']['replicas'] = 0
        self.save_yaml(green_recycle, os.path.join(blue_dir, f'Deployment/[green]{deploy_name}.yaml'))
        blue_svc = self.generate_blue_green_service(self.blue_name)
        ...
        for idx, ingress_name in enumerate(self.resource_names['ingress']):
            ...
            self.save_yaml(self.generate_ingress(base_ingress, 'user_switch', self.blue_name),
                           os.path.join(blue_dir, f'Ingress/[blue]user_switch_{ingress_name}.yaml'))
            self.save_yaml(self.generate_ingress(base_ingress, 'uid_switch', self.blue_name),
                           os.path.join(blue_dir, f'Ingress/[blue]uid_switch_{ingress_name}.yaml'))
            self.save_yaml(self.generate_ingress(base_ingress, 'nouid_switch', self.blue_name),
                           os.path.join(blue_dir, f'Ingress/[blue]nouid_switch_{ingress_name}.yaml'))
            self.save_yaml(self.generate_ingress(base_ingress, 'all_switch', self.blue_name),
        ...
        blue_recycle = self.generate_blue_green_deployment(self.blue_name)
        blue_recycle['spec']['replicas'] = 0
        self.save_yaml(blue_recycle, os.path.join(green_dir, f'Deployment/[blue]{deploy_name}.yaml'))
        green_svc = self.generate_blue_green_service(self.green_name)
        self.save_yaml(green_svc, os.path.join(green_dir, f'Service/[green]{svc_name}.yaml'))
        for idx, ingress_name in enumerate(self.resource_names['ingress']):
            base_ingress = self.ingresses[idx]
            self.save_yaml(self.generate_ingress(base_ingress, 'user_switch', self.green_name),
                           os.path.join(green_dir, f'Ingress/[green]user_switch_{ingress_name}.yaml'))
            self.save_yaml(self.generate_ingress(base_ingress, 'uid_switch', self.green_name),
                           os.path.join(green_dir, f'Ingress/[green]uid_switch_{ingress_name}.yaml'))
            self.save_yaml(self.generate_ingress(base_ingress, 'nouid_switch', self.green_name),
                           os.path.join(green_dir, f'Ingress/[green]nouid_switch_{ingress_name}.yaml'))
            self.save_yaml(self.generate_ingress(base_ingress, 'all_switch', self.green_name),
        ...
        blue_count = count_files(os.path.join(output_dir, self.blue_name))
        green_count = count_files(os.path.join(output_dir, self.green_name))
        base_count = count_files(os.path.join(output_dir, 'mse-base'))
        print(f"\n✅ 所有资源文件已生成到: {output_dir}")
        print(f"   {self.blue_name} 环境: {blue_count} 个文件")
        print(f"   {self.green_name} 环境: {green_count} 个文件")
    ...
    def _copy_global_var_files(self, output_dir: str):
        ...
        if os.path.exists(self.bg_xls_path):
            for env in [self.blue_name, self.green_name]:
